...
function Common.getPathTable(path: string): { string }
	return string.split(path, ".")
end

function Common.getPathTablePointer(data: { [any]: any }, pathTable: { string }, create: boolean?): ({ [any]: any }?, string?)
	local pointer = data
...
function Common.getPathTablePointerCreate(data: { [any]: any }, pathTable: { string }): ({ [any]: any }, string)
	local pointer, index = Common.getPathTablePointer(data, pathTable, true)
	return pointer :: {[any]: any}, index :: string
end
...
function Common.getPathPointer(data: { [any]: any }, path: string): ({ [any]: any }, string)
...
function Common.cleanSignalTable(signals: { [string]: any }, pathTable: {string}, signalTable: {[string]: any}): ()
...
function Common.connectReplicaSignal(self: any, signalName: string, path: string, listener: (...any) -> ())
	...
	if path == "" then
		...
		signalTable = Common.getPathPointer(listeners, `{path}.{signalName}`)
	end
...
function Common.fireReplicaSignal(self: any, signalName: string, pathTable: {string}?, index: string?, ...: any): ()
...
function Common._onSetValue(self: any, pathTable: {string}, pointer:{[string]: any}, index: string, value: any): ()
...
function Common.onSetValue(self: any, path: string, value: any): ()
...
function Common.onSetValues(self: any, path: string, values: { [string]: any }): ()
	local pathTable = Common.getPathTable(path)
	local pointer, index = Common.getPathTablePointerCreate(self.Data, pathTable)
...
function Common.onArrayInsert(self: any, path: string, index: number, value: any): ()
	local pathTable = Common.getPathTable(path)
...
function Common.onArraySet(self: any, path: string, index: number, value: any): ()
	local pathTable = Common.getPathTable(path)
...
function Common.onArrayRemove(self: any, path: string, index: number): ()
	local pathTable = Common.getPathTable(path)
	local pointer, _index = Common.getPathTablePointerCreate(self.Data, pathTable)
